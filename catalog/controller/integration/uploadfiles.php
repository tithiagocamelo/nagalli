...
class ControllerIntegrationUploadfiles extends Controller {
		...
		foreach ($this->files as $file) {
			...
			foreach ($xml as $productXML) {
				...
				if (isset($productDB['product_id'])) {
                    ...
                    $productDB['date_added'] = $product->date_added;
	...
	private function relcategorias()
	{
		$ids = array();
		...
		foreach ($this->files as $file) {
			...
			foreach ($xml as $relactedXML) {
				
				$ids['product_related'][] = array(
					'relacted_id' => (int)$relactedXML->related_id,
					'product_id' => (int)$relactedXML->product_id
				);				
		...
		$this->load->model('integration/relacted');

		$this->model_integration_relacted->addRelacted($ids);
