...
class ModelSaleReturn extends Model {
	...
	public function getReturn($return_id, $new = false) {
		...
		if($new) {
			$query_detreturn = $this->db->query("SELECT * FROM ". DB_PREFIX . "detreturn WHERE return_id = '" . (int)$return_id . "'");

			$dados['produtos'] = $query_detreturn->rows;
		}
