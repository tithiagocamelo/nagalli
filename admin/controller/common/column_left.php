	...
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			*/	
			...
			//if ($this->user->hasPermission('access', 'localisation/mensagem')) {
				$marketing[] = array(
					'name'	   => $this->language->get('text_mensagem'),
					'href'     => $this->url->link('localisation/mensagem', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			//}
