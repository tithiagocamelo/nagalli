...
class ControllerReportProductPurchased extends Controller {
	public function index() {
		...
		if (isset($this->request->get['order']) and $this->request->get['order'] == 'ASC') {
			$url .= '&order=DESC';
		} else {
			$url .= '&order=ASC';
		}
		
		$data['sort_order'] = $this->url->link('report/product_purchased', 'token=' . $this->session->data['token'] . '&sort=op.quantity' . $url, true);
