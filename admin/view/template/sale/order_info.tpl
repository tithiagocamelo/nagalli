        ...
        <div class="panel panel-default">
          ...
          <table class="table">
            <tbody>
              ...
              <?php } ?>
              <?php if (isset($cod_correios)) { ?>
                <tr>
                  <td><button data-toggle="tooltip" title="<?php echo $text_shipping_method; ?>" class="btn btn-info btn-xs"><i class="fa fa-truck fa-fw"></i></button></td>
                  <td><?php echo 'Cód. Log. Reversa: '. $cod_correios; ?></td>
                </tr>
              <?php } ?>
            </tbody>
          </table>
        ...
        <div class="tab-content">
          <div class="tab-pane active" id="tab-history">
            ...
            <fieldset>
              ...
              <?php //print '<pre>';var_dump($order_statuses); ?>
              <form class="form-horizontal">
                <div class="form-group">
                  <label class="col-sm-2 control-label" for="input-order-status"><?php echo $entry_order_status; ?></label>
                  <div class="col-sm-10">
                    <select name="order_status_id" id="input-order-status" class="form-control">
                      <?php foreach ($order_statuses as $order_statues) { ?>
                      <?php if ($order_statues['order_status_id'] == $order_status_id) { ?>
                      <option value="<?php echo $order_statues['order_status_id']; ?>" selected="selected" data-mensagem_id="<?php echo $order_statues['mensagem_id']; ?>"><?php echo $order_statues['name']; ?></option>
                      <?php } else { ?>
                      <option value="<?php echo $order_statues['order_status_id']; ?>" data-mensagem_id="<?php echo $order_statues['mensagem_id']; ?>"><?php echo $order_statues['name']; ?></option>
                      <?php } ?>
                      <?php } ?>
                    </select>
                  </div>
                ...
                <div class="form-group" id="to-mensagem">
                  <label class="col-sm-2 control-label" for="input-mensagem"><?php echo $entry_mensagem; ?></label>
                  <div class="col-sm-10">
                    <select name="mensagem" id="input-mensagem" class="form-control">
                      <option value=""> -- Selecione um modelo de mensagem -- </option>
                      <?php if(isset($mensagens) and is_array($mensagens)) { foreach ($mensagens as $mensagem) { ?>
                      <option value="<?php echo $mensagem['mensagem_id']; ?>"><?php echo $mensagem['titulo']; ?></option>
                      <?php } } ?>
                    </select>
                  </div>
                </div>
...
<script>
  $(document).ready(function() {
    ...
    $('#input-mensagem').change(function(){ 
      
      if( $(this).val() ) {
        
        $.ajax({
          url: unescapeHtml('<?php print isset($message_link) ? $message_link : ''; ?>'),
          type: 'post',
          data: { mensagem_id: $(this).val() },
          success: function(result) {
            $('.summernote').summernote('code', unescapeHtml(result.data.mensagem))
          }
        })
    ...
    $('#input-order-status').change(function() {
      ...
      var mensagem_id = $('#input-order-status').find('option[value='+ $('#input-order-status').val() +']').data('mensagem_id')

      $('#input-mensagem').val(mensagem_id)

      $('#input-mensagem').trigger('change')
